...
<template>
    <div>
        <h1 class="title flex justify-between items-center">
            FAQ
            <fwb-button href="/admin/faq/create" color="default">Додати</fwb-button>
        </h1>
        <fwb-table hoverable class="mb-4">
            ...
            <fwb-table-body>
                ...
                <fwb-table-row v-for="item in faq" :key="item.id">
        ...
        <fwb-pagination v-model="currentPage" :total-items="faq.length" show-icons previous-label=""
...
export default {
    ...
    methods: {
        deleteItem(item, index) {
            axios.post('/admin/faq-delete/' + item.id)
                .then(() => {
